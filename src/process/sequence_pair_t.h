...
public:
    ...
    //static variables
    static vector<edge_t> connections; //all edges
    static vector<vector<int>> connectivities; //VE graph
    ...
    static unordered_map<const module_t*, int> soft_module_to_id_m; //from the module to its seq# (for building the connectivities graph)
    static unordered_map<const module_t*, int> fix_module_to_id_m;//from the module to its seq# (for building the connectivities graph)
    ...
    //essential
    pair<bool, vector<vec2d_t>> find_position(int,int); // verify if the current sequence pair form the legal position
