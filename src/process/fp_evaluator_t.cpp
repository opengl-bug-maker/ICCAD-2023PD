...
uint32_t fp_evaluator_t::get_dead_space(floorplan_t& fp) {
    vector<rect_t> empty_rects = fp.get_polygon_forest().get_empty_spaces();
    uint32_t dead_area = 0;
    for(auto& empty_rect:empty_rects){
        if(empty_rect.get_size().get_x()<floorplan_t::min_w_h[0] || empty_rect.get_size().get_y()<floorplan_t::min_w_h[1]){
